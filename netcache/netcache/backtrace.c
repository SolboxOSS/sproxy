...
#define 	CONTEXT_DEFINED_64BIT
#pragma message ("StackWalk redefined to StackWalk64")
#endif
...
static void
output_to_eventlog(char *message)
{
#if 0
	char	msg[10];
	HANDLE 	heventsource;
	LPCSTR 	perrormsgtemp;
	
	fputs(message, stderr);
	//_sntprintf(msg, CountOf(message), _T("%s"), message);
	//sprintf(msg, sizeof(msg), "%s", message);

	heventsource = RegisterEventSource(NULL, "BACKTRACE");
	if (heventsource != NULL) {
		perrormsgtemp = message;
		if (!ReportEvent(heventsource, 
						...
						&perrormsgtemp,
						NULL)) {
			fprintf(stderr, "Could not lot to event log\n");
		}
