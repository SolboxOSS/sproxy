...
AC_DEFUN([NC_DEFAULTS],[

    AC_ARG_VAR([dav_user],
        [if invoked by root, mount.davfs runs as this user [netcache]])
    if test -z "$dav_user"; then dav_user="netcache"; fi

    AC_ARG_VAR([dav_group],
        [the group, the mount.davfs daemon belongs to [netcache]])
    if test -z "$dav_group"; then dav_group="netcache"; fi
...
Configuration for building netcache AC_PACKAGE_VERSION:

  Install Prefix:            ${prefix}
  Compiler:                  ${CC}
  Large File Support:        ${dav_lfs}
  Memory Overflow Check:     ${overflowcheck}
